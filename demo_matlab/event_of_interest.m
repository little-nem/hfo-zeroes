...
%   time_threshold  = Minimum time between two distincts events (events whose
%                       temporal distance is below this threshold are merged)
%
% output
%   event_list      = Array of time at which event of interest occurs 
    ...
    filter = fir1(64, [band_down/half_fs band_up/half_fs]);

    filtered_s = filtfilt(double(filter), 1, double(s));
    ...
    % Clean idx: make it sparse (keep only the beginning of the event, the 
    % maximum duration of a single event being defined as time_threshold)
